...
async def get_accuracy_report(
    ...
    """Return accuracy report metadata for a plan (scores, fix counts, per-MVP breakdown)."""
    try:
        return engine.get_accuracy(plan_id)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))


@router.get("/{plan_id}/accuracy/report")
async def download_accuracy_report(
    plan_id: str,
    user: dict = Depends(get_current_user),
    engine=Depends(get_migration_engine),
):
    """Download the MIGRATION_ACCURACY.md for a plan."""
    try:
        md = engine.get_accuracy_report_md(plan_id)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    if not md:
        raise HTTPException(status_code=404, detail="No accuracy report found for this plan")
    return Response(
        content=md,
