...
class MigrationPlan(Base):
    ...
    target_manifest = Column(JSONB, nullable=True)                  # [{source_type, count, target_language, target_artifact, target_detail, confirmed}]
...
class RAGFeedback(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_rag_feedback_project", "project_id", "created_at"),
    ...
    node_annotations = relationship(
        "RAGFeedbackNode", back_populates="feedback",
        cascade="all, delete-orphan",
    )

    def __repr__(self):
        return f"<RAGFeedback(feedback_id={self.feedback_id}, rating={self.rating}, helpful={self.helpful})>"
...
class RAGFeedbackNode(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    feedback_id = Column(
        UUID(), ForeignKey("rag_feedback.feedback_id", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    def __repr__(self):
        return f"<RAGFeedbackNode(id={self.id}, node_id='{self.node_id}', relevant={self.was_relevant})>"


class RAGFeedbackInsight(Base):
    """Pre-computed analysis insights from FeedbackAnalyzer.

    Rows expire after expires_at and are regenerated by a background worker.
    """
    __tablename__ = "rag_feedback_insights"
    __table_args__ = (
        Index("ix_rag_feedback_insights_project", "project_id", "created_at"),
    )
    ...
    def __repr__(self):
        return f"<RAGFeedbackInsight(insight_id={self.insight_id}, type='{self.analysis_type}', metric='{self.metric_name}')>"


class RAGAdaptiveSetting(Base):
    """Adaptive retrieval parameters derived from feedback analysis.

    Rows are looked up by project_id where expires_at > NOW().
    The most recent non-expired row wins.
    """
    __tablename__ = "rag_adaptive_settings"
    __table_args__ = (
        Index("ix_rag_adaptive_project", "project_id", "intent", "created_at"),
    )
    ...
    def __repr__(self):
        return f"<RAGAdaptiveSetting(setting_id={self.setting_id}, project_id={self.project_id}, top_k={self.top_k})>"
