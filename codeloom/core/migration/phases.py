...
def _build_agentic_task_prompt(
    ...
    # For test phase: inject transform output file manifest so the agent knows what to test
    if context_type == "test":
        transform_files = plan.get("_transform_output_files", [])
        if transform_files:
            manifest = "\n".join(
                f"  - {f.get('file_path', 'unknown')}  [{f.get('language', '')}]"
                for f in transform_files[:60]
            )
            parts.append(
                f"## Files Generated by Transform Phase\n"
                f"You MUST generate tests for the following {len(transform_files)} file(s):\n\n"
                f"{manifest}"
            )
...
_PHASE_INSTRUCTIONS: Dict[str, str] = {
    ...
    "test": (
        ...
        "Steps:\n"
        "1. Review the 'Files Generated by Transform Phase' list above — each file needs tests.\n"
        "   If no list is present, use get_source_code to find the migrated files.\n"
        "2. Use get_source_code to read the ORIGINAL implementation for business logic reference.\n"
        "3. Use get_functional_context to understand the business rules and edge cases.\n"
        "4. Use lookup_framework_docs for the target test framework patterns (e.g., JUnit 5, xUnit, Jest).\n"
        "5. Generate one test file per source file — name it <OriginalName>Test or <OriginalName>.spec.\n"
        ...
        "- DO NOT return an empty array [] — if you cannot generate tests, explain why in a comment\n"
        "  inside a placeholder test file instead."
    ),
    "discovery": (
...
def _parse_json_files(raw: str) -> List[Dict]:
    ...
    try:
        ...
        if isinstance(parsed, list):
            if parsed:
                return parsed
            # Valid JSON empty array — agent produced nothing, no warning needed
            logger.debug("LLM output was a valid empty array []")
            return []
    except json.JSONDecodeError:
        pass
